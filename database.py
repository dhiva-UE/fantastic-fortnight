...
def create_tables_sqlite(cursor):
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS testing_checklists (
            ...
            FOREIGN KEY (project_id) REFERENCES testing_projects(project_id),
            FOREIGN KEY (source_checklist_id) REFERENCES testing_checklists(checklist_id)
        )
    """)

    cursor.execute("""
        CREATE TABLE IF NOT EXISTS testing_checklist_items (
            checklist_item_id INTEGER PRIMARY KEY AUTOINCREMENT,
            checklist_id INTEGER NOT NULL,
            component_id INTEGER NOT NULL,
            issued_to TEXT,
            status TEXT DEFAULT 'Out for Testing',
            remarks TEXT,
            returned_at TEXT,
            created_at TEXT DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (checklist_id) REFERENCES testing_checklists(checklist_id),
...
def create_tables_postgresql(cursor):
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS testing_checklists (
            checklist_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            project_id INTEGER NOT NULL REFERENCES testing_projects(project_id),
            checklist_name TEXT NOT NULL,
            test_date TEXT NOT NULL,
            remarks TEXT,
            created_by TEXT,
            created_at TEXT DEFAULT CURRENT_TIMESTAMP,
            source_checklist_id INTEGER REFERENCES testing_checklists(checklist_id)
        )
    """)

    cursor.execute("""
        CREATE TABLE IF NOT EXISTS testing_checklist_items (
            checklist_item_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            checklist_id INTEGER NOT NULL REFERENCES testing_checklists(checklist_id),
            component_id INTEGER NOT NULL REFERENCES products(product_id),
            issued_to TEXT,
            status TEXT DEFAULT 'Out for Testing',
            remarks TEXT,
            returned_at TEXT,
...
def get_or_create_supplier_id(supplier_name):
    normalized_name = normalize_text(supplier_name) or "Imported Supplier"
    existing = execute(
        "SELECT supplier_id FROM suppliers WHERE LOWER(supplier_name) = LOWER(?) LIMIT 1",
        (normalized_name,),
        fetchone=True,
    )
    if existing:
    ...
    created = execute(
        "SELECT supplier_id FROM suppliers WHERE LOWER(supplier_name) = LOWER(?) ORDER BY supplier_id DESC LIMIT 1",
        (normalized_name,),
        fetchone=True,
    )
...
def import_components(rows):
    ...
    for index, row in enumerate(rows, start=2):
        try:
            ...
            reorder_level_raw = row.get("reorder_level", 5)
            supplier_name = normalize_text(row.get("supplier_name") or row.get("supplier"))
            assigned_to = normalize_text(row.get("assigned_to"))

            price = 0 if price_raw in (None, "") else require_non_negative_number(price_raw, "Price")
            reorder_level = 5 if reorder_level_raw in (None, "") else require_positive_int(reorder_level_raw, "Reorder level")
...
# ---------------- TESTING ----------------
def add_testing_project(project_name, description="", created_by=""):
    execute(
        """
        INSERT INTO testing_projects (project_name, description, created_by)
        VALUES (?, ?, ?)
        """,
        (require_text(project_name, "Project name"), normalize_text(description), normalize_text(created_by)),
    )
...
def get_testing_projects():
    return read_dataframe("""
        SELECT tp.project_id,
               ...
               tp.created_by,
               tp.created_at,
               COUNT(DISTINCT tc.checklist_id) AS checklist_count
        FROM testing_projects tp
        LEFT JOIN testing_checklists tc ON tp.project_id = tc.project_id
        GROUP BY tp.project_id, tp.project_name, tp.description, tp.created_by, tp.created_at
        ORDER BY tp.project_name
    """)


def add_testing_checklist(project_id, checklist_name, test_date, remarks="", created_by="", source_checklist_id=None):
    conn = get_connection()
    try:
        cursor = conn.cursor()
        normalized_name = require_text(checklist_name, "Checklist name")
        ...
        if using_postgresql():
            cursor.execute(
                adapt_query("""
                    INSERT INTO testing_checklists (
                        project_id, checklist_name, test_date, remarks, created_by, source_checklist_id
                    )
                    VALUES (?, ?, ?, ?, ?, ?)
                    RETURNING checklist_id
                """),
                (project_id, normalized_name, require_text(test_date, "Test date"), normalized_remarks, normalized_created_by, source_checklist_id),
            )
        ...
        if source_checklist_id:
            cursor.execute(
                adapt_query("""
                    INSERT INTO testing_checklist_items (
                        checklist_id, component_id, issued_to, status, remarks, returned_at
                    )
                    SELECT ?, component_id, issued_to, 'Out for Testing', remarks, NULL
                    FROM testing_checklist_items
                    WHERE checklist_id = ?
                """),
                (checklist_id, source_checklist_id),
            )
...
def get_testing_checklists():
    return read_dataframe("""
        SELECT tc.checklist_id,
               ...
               tc.created_by,
               tc.created_at,
               tc.source_checklist_id,
               source_tc.checklist_name AS source_checklist_name,
               COUNT(tci.checklist_item_id) AS item_count
        FROM testing_checklists tc
        INNER JOIN testing_projects tp ON tc.project_id = tp.project_id
        LEFT JOIN testing_checklists source_tc ON tc.source_checklist_id = source_tc.checklist_id
        LEFT JOIN testing_checklist_items tci ON tc.checklist_id = tci.checklist_id
        GROUP BY tc.checklist_id, tc.project_id, tp.project_name, tc.checklist_name, tc.test_date,
                 tc.remarks, tc.created_by, tc.created_at, tc.source_checklist_id, source_tc.checklist_name
        ORDER BY tc.test_date DESC, tc.checklist_id DESC
    """)


def add_testing_checklist_item(checklist_id, component_id, issued_to="", status="Out for Testing", remarks=""):
    execute(
        """
        INSERT INTO testing_checklist_items (checklist_id, component_id, issued_to, status, remarks, returned_at)
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (
            checklist_id,
            component_id,
            normalize_text(issued_to),
            require_text(status, "Status"),
            normalize_text(remarks),
            current_timestamp() if require_text(status, "Status") in {"Returned OK", "Damaged", "Lost"} else None,
        ),
    )
...
def update_testing_checklist_item(checklist_item_id, issued_to="", status="Out for Testing", remarks=""):
    normalized_status = require_text(status, "Status")
    execute(
        """
        UPDATE testing_checklist_items
        SET issued_to = ?, status = ?, remarks = ?, returned_at = ?
        ...
        (
            ...
            normalized_status,
            normalize_text(remarks),
            current_timestamp() if normalized_status in {"Returned OK", "Damaged", "Lost"} else None,
            checklist_item_id,
        ),
    )


def delete_testing_checklist_item(checklist_item_id):
    execute("DELETE FROM testing_checklist_items WHERE checklist_item_id = ?", (checklist_item_id,))


def get_testing_checklist_items():
    return read_dataframe("""
        SELECT tci.checklist_item_id,
               tci.checklist_id,
               tc.project_id,
               tp.project_name,
               tc.checklist_name,
               ...
               tci.returned_at,
               tci.created_at
        FROM testing_checklist_items tci
        INNER JOIN testing_checklists tc ON tci.checklist_id = tc.checklist_id
        INNER JOIN testing_projects tp ON tc.project_id = tp.project_id
        INNER JOIN products p ON tci.component_id = p.product_id
        ORDER BY tc.test_date DESC, tci.checklist_item_id DESC
    """)
